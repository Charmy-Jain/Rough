...
function HomePage() {
  ...
  return (
    <div className="flex h-screen w-screen bg-black text-white">
      ...
      {/* Left Sidebar (Chat List + Settings Menu) */}
      <div className={`relative w-1/4 md:w-1/5 bg-gray-900 border-r border-gray-700 overflow-y-auto transition-all duration-300 ${showSidebar ? "w-2/5" : "w-1/4"}`}>
        ...
        {/* Header Section (Logo + Chats + ☰ Button) */}
        <div className="p-4 flex justify-between items-center bg-gray-900 sticky top-0 z-10">
          <div className="flex items-center">
            <img src={logo} alt="Chat Logo" className="w-10 h-10 mr-2 rounded" />
        ...
        {/* Search Bar */}
        <div className="p-4">
          <input type="text" placeholder="Search..." className="w-full px-3 py-2 bg-gray-800 rounded-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-green-500" />
        </div>

        {/* Chat List */}
        {chats.map((chat) => (
          <div key={chat.id} className={`flex items-center p-3 cursor-pointer hover:bg-gray-800 ${activeChat && activeChat.id === chat.id ? 'bg-green-900' : ''}`} onClick={() => handleChatClick(chat.id)}>
